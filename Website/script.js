...
function runHeroEntrance(){
  ...
  if(spk){
    ...
    spk.style.transition=isMobile
      ? 'stroke-dashoffset .75s cubic-bezier(.22,.61,.36,1) .25s'
      : 'stroke-dashoffset 1.8s cubic-bezier(.4,0,.2,1) 1s';
  ...
  /* Hero card counters after card appears.
     Mobile keeps the animation, but with shorter durations and lighter stagger. */
  setTimeout(()=>{
    ...
    if(isMobile){
      if(c1)counter(c1,487.4,'€','K',1,780);
      setTimeout(()=>{if(c2)counter(c2,24.3,'','%',1,700);},90);
      setTimeout(()=>{if(c3)counter(c3,1.87,'','',2,680);},170);
      setTimeout(()=>{if(c4)counter(c4,23,'','',0,620);},240);
      return;
    }
...
document.querySelectorAll('.qpill').forEach(pill=>{
  pill.addEventListener('click',function(){
    const currentLang = (window.__getLang ? window.__getLang() : (localStorage.getItem('elara_lang') || 'it'));
    const d=chatData[currentLang]?.[this.dataset.q]||chatData.it[this.dataset.q];
    if(!d)return;
...
document.getElementById('ctaForm').addEventListener('submit',function(e){
  ...
  const currentLang = (window.__getLang ? window.__getLang() : (localStorage.getItem('elara_lang') || 'it'));
  alert(currentLang==='en'?`Thanks! Added to waitlist: ${email}`:`Grazie! Aggiunto alla lista d'attesa: ${email}`);
  this.reset();
});
...
const i18n={
  it:{
    toggle:'EN',
    navLinks:['Funzionalità','Chi siamo','Tecnologia','Sicurezza','Pricing'],navCta:'Inizia ora',
  ...
  en:{
    toggle:'IT',
    navLinks:['Features','About us','Technology','Security','Pricing'],navCta:'Get started',
